...
public class AI {
    ...
    public boolean CorrectPlacement(List<List<int[]>> ships, int x, int y, int orientation,int shipsize) {
        if (orientation == 0){
    ...
    public List<List<int[]>> GenerateBoard()
    {
        ...
        for (int sizeship : allship) {
            ...
            while (!correctplacement) {
                ...
                correctplacement = CorrectPlacement(toreturn,x,y,orientation,sizeship);
                if (correctplacement) { PlaceShip(toreturn,x,y,orientation,sizeship); }
            }
        }
        return toreturn;
    };
